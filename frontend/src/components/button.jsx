const Bouton = ({ variant, text, link }) => {
    ...
    return (
        <div>
            <a href={link}>
                <button className={`${getBackgroundClass(variant)} border py-2 sm:text-sm lg:text-base px-4 sm:px-4 lg:px-8 rounded-full text-white font-bold font-Roboto transition duration-200 linear`}>
                    {text}
                </button>
            </a>
        </div>
